...
- **Menu entries show their shortcut** across every menu — Windows, Sessions, Bookmarks, Options and the panel context menu — in a dimmed column on the right, so the keyboard route is discoverable from the menu rather than only from the help panel. Commands, whose hotkey used to be glued onto the label as ` [Ctrl+K]`, now use that same column. Only the primary key is shown; the full list stays in Settings and in Help.
...
- **Buttons in Settings could not be clicked on a localised build.** The renderer drew the `[ label ]` boxes character by character while hit-testing measured them in *bytes*, so every button past the first sat some columns away from where clicks were expected — "Сбросить" was unreachable by mouse. Both now derive their positions from one shared calculation.
- **"Reset to Defaults" left the Keybindings tab empty** until the modal was reopened. It installed a bare `Config::default()`, whose bindings are all unset; the defaults are now filled in straight away, so the new values are visible immediately.
- **A long field label ran into its own value.** The label column was fixed at 32 characters, which the longest English label already filled and a translated one overran. It is now measured from the labels in use, with a gap that survives on narrow layouts.
- **Settings showed only the first key of a multi-key binding.** `Close panel` is bound to `Alt+W`, `Alt+X` and `F10`, but the list displayed `Alt+W` alone — and `Delete` then removed all three at once, two of them invisible. Every alternative is now listed, and `Delete` removes them one at a time (`Shift+Delete` clears the action outright), which also makes a multi-key binding editable at all: the capture dialog can only add.
