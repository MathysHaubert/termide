...
// TermIDE fork of the upstream `unicode-width` 0.2.2 tables. The only change
// is `bengali_spacing_mark_override` below: glibc `wcwidth()` gives the
// Bengali spacing marks width 1 while Unicode marks two of them
// Grapheme_Extend (width 0), which desynced the cursor in bn/hi locales.
// Everything else — the trie tables and the sequence state machine (emoji
// presentation, ZWJ, modifiers, regional indicators, ligatures) — is
// upstream, because ratatui's diff must agree with the host terminal.
...
/// The sequence state a variation selector starts, or none when the host
/// terminal ignores selectors for width (see
/// [`crate::set_variation_selectors_change_width`]).
#[inline]
fn variation_selector_info(info: WidthInfo) -> WidthInfo {
    if crate::variation_selectors_change_width() {
        info
    } else {
        WidthInfo::DEFAULT
    }
...
/// Bengali spacing marks that glibc `wcwidth()` (and terminals following it)
/// advance the cursor for, even though two of them (U+09BE, U+09D7) are
/// `Grapheme_Extend` and therefore zero-width in UAX #11 terms.
#[inline]
fn bengali_spacing_mark_override(c: char) -> bool {
    matches!(
        c,
        '\u{09BE}'
            | '\u{09BF}'
...
fn lookup_width(c: char) -> (u8, WidthInfo) {
    if bengali_spacing_mark_override(c) {
    ...
    if width < 3 {
        ...
        match c {
            ...
            '\u{FE0E}' => (0, variation_selector_info(WidthInfo::VARIATION_SELECTOR_15)),
            '\u{FE0F}' => (0, variation_selector_info(WidthInfo::VARIATION_SELECTOR_16)),
